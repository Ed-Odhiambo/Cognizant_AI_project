...
def load_data(path: str = "/path/to/csv/"):
    ...
    df = pd.read_csv(f"{'sensor_stock_levels.csv'}")
    df.drop(columns=["Unnamed: 0"], inplace=True, errors='ignore')
    """
    This meant to drop the Unnamed: 0 column that appears in the datasets
    and is similar to the index column and thus redundant
    """
    return df
...
def run():
    """
    This function executes the pipeline of loading the prepared
